...
import FavoriteButton from "@pages/popup/components/FavoriteButton";

const Popup = () => {
  const [value, setValue] = useState("");
  const [openedBookmark, setOpenedBookmark] = useState<string | undefined>(null);
  const [searchOptions, setSearchOptions] = useState([]);
  const [displayTopSites, setDisplayTopSites] = useState<undefined | chrome.topSites.MostVisitedURL[]>(undefined);
  const [displayBookmarks, setDisplayBookmarks] = useState<undefined | chrome.bookmarks.BookmarkTreeNode[]>(undefined);
  ...
  useEffect(() => {
    ...
    chrome.permissions.contains(
      ...
      (result) => {
        if (result) {
          chrome.bookmarks.getTree(setDisplayBookmarks);
        }
      },
    );
  }, []);
  ...
  const toggleBookmarks = (enabled: boolean) => {
    if (enabled) {
      ...
      <div className="block fixed w-full top-0">
        <div className="flex flex-col w-full">
          ...
          <div className="flex flex-row justify-end w-full">
            <div className="flex flex-row px-2 py-3 gap-2">
              <div className="text-base text-blue-700 dark:text-blue-400">Show bookmarks</div>
              <Switch
                checked={!!displayBookmarks}
                onChange={toggleBookmarks}
                className={`${
                  !!displayBookmarks ? "bg-blue-600" : "bg-gray-200"
                } relative inline-flex h-6 w-11 items-center rounded-full`}
              >
                <span className="sr-only">Show bookmarks</span>
                <span
                  className={`${
                    !!displayBookmarks ? "translate-x-6" : "translate-x-1"
                ...
                {displayTopSites.slice(0, 10).map((o) => (
                  <li key={"top-sites" + o.url} className="rounded-md hover:bg-gray-100 p-2 dark:hover:bg-slate-600">
                    ...
                    >
                      ...
                      <span className="text-ellipsis overflow-hidden whitespace-nowrap text-center dark:text-white">
                        {o.title}
                      </span>
                    </a>
                  </li>
